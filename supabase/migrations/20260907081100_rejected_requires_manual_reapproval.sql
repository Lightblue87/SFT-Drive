-- Eine admin-seitige Ablehnung ist eine bewusste Entscheidung für genau diese
-- Tour und darf durch eine Neuanmeldung nicht einfach automatisch überschrieben
-- werden — auch nicht, wenn die Tour im Automatik-Modus läuft. Ein zuvor
-- `rejected` User kann sich zwar erneut anmelden (kontrollierte Reaktivierung,
-- §8.7), landet dabei aber unabhängig von `confirmation_mode` und freier
-- Kapazität immer zunächst wieder bei `pending` und braucht erneut eine
-- explizite Admin-Bestätigung über `approve_tour_registration`. Eine
-- Stornierung durch den User selbst (`cancelled`) ist davon nicht betroffen
-- und folgt weiterhin normal dem Automatik-/Manuell-Modus der Tour.
...
begin
  ...
  if v_existing_found and v_existing.status = 'rejected' then
    ...
    -- immer erneut pending, nie automatisch confirmed/waitlisted.
