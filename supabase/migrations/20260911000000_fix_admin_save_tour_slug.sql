-- Fix: admin_save_tour() (20260910020000) gave every tour created from the
-- macOS app a raw UUID-based slug ("tour-3fa85f64...") instead of the
-- mandated slugify(title)+start_date scheme (§8.3 "Umsetzungsentscheidung:
-- Slug ist kein Admin-Eingabefeld"). Tours created via the PWA keep readable
-- slugs; tours created via the Mac app did not — two divergent slug schemes
-- in the same product. This makes the RPC derive the slug itself the same
-- way, with the same numeric-suffix collision handling the PWA already uses
-- client-side on a rare title+date collision (§8.3: "wird serverseitig
-- automatisch durch einen angehängten numerischen Suffix aufgelöst").
--
-- Only the insert (create) path is affected — an existing tour's slug is
-- never touched here, matching "Bei bestehenden Touren bleibt der einmal
-- vergebene Slug beim Bearbeiten unangetastet" (§8.3).
create or replace function public.slugify_text(p_text text)
returns text language sql immutable set search_path = public
as $$
  select trim(both '-' from regexp_replace(
    lower(translate(
      replace(replace(replace(coalesce(p_text, ''), 'Ä', 'Ae'), 'Ö', 'Oe'), 'Ü', 'Ue'),
