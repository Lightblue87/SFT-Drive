-- Ergänzt tour_hotel_suggestions um einen optionalen Preis pro Nacht sowie
-- einen optionalen Endtermin, damit ein Hotelvorschlag mehrere aufeinander
-- folgende Übernachtungen abdecken kann, ohne für jede Nacht denselben
-- Eintrag mit allen Details zu wiederholen (ausdrücklicher Wunsch des
-- Projektinhabers, 11.09.2026).
--
-- Bewusste Abweichung von der bisherigen Formulierung in CLAUDE.md §36.3/
-- §36.12 ("keine Preise speichern"): dort ging es um persönliche
-- Buchungs-/Zahlungsdaten des Teilnehmers (Reservierungsnummer, tatsächlich
-- gezahlter Betrag, Zahlungsmittel). price_per_night ist davon zu
-- unterscheiden -- es ist eine organisatorische Angabe des Admins zum
-- Hotelvorschlag selbst (der vom Hotel genannte Preis pro Nacht, analog zu
-- Name/Adresse/Buchungsfrist), keine Zahlungs- oder Reservierungsdaten
-- einzelner Teilnehmer. CLAUDE.md wird entsprechend nachgezogen.
--
-- night_date bleibt die erste abgedeckte Nacht; night_date_end ist optional
-- und bezeichnet die letzte abgedeckte Nacht (NULL = nur night_date selbst,
-- wie bisher). Bewusst nullable statt NOT NULL mit Backfill, damit bereits
-- bestehende Admin-Schreibpfade (admin_save_tour_resource, PWA-Direktinsert
-- ohne dieses Feld) unverändert weiterfunktionieren -- Anwendungscode
-- behandelt "night_date_end fehlt" als "deckt nur night_date ab" via
-- coalesce(night_date_end, night_date).
alter table public.tour_hotel_suggestions
  add column night_date_end date null,
  add column price_per_night numeric(8,2) null;

alter table public.tour_hotel_suggestions
  add constraint tour_hotel_suggestions_night_end_chk
  check (night_date_end is null or night_date_end >= night_date);

alter table public.tour_hotel_suggestions
  add constraint tour_hotel_suggestions_price_chk
  check (price_per_night is null or price_per_night >= 0);

-- admin_save_tour_resource() (Mac-App, §40.3) erlaubt die beiden neuen Felder
-- für 'hotels' und validiert sie serverseitig gegen den Tourzeitraum, analog
-- zu night_date selbst.
