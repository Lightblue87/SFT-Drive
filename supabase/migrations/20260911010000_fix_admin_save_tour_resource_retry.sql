-- Fix: admin_save_tour_resource() (20260910030000) claimed client-generated
-- IDs made retries safe, but a genuine retry after an ambiguous network
-- failure (the first call's INSERT already committed, the response was
-- lost) always passed p_expected=null and the row now existed — so the
-- retry fell into the CONFLICT branch ("Eintrag wurde inzwischen
-- verändert") even though nothing had actually changed, it was the
-- caller's own prior success. admin_create_restaurant (20260910050000)
-- hits this directly: it calls this function three times in sequence with
-- p_expected always null, and RestaurantImportReviewView.swift reuses the
-- same client-generated stopID across retries.
--
-- Fix: when p_expected is null and a row with this ID already exists, only
-- reject as a conflict if the existing row's values actually differ from
-- what's being written. If every field in p_values already matches the
-- existing row, this is the same create being safely retried — return
-- success instead of a false CONFLICT. A genuinely different row that
-- happens to reuse the same client-generated ID (a real, not merely
-- ambiguous, collision) still gets validated below and would fail there
-- on a real mismatch; this only short-circuits the case where the values
-- are already byte-identical.
...
begin
  ...
  if old_row is not null then
    ...
    elsif p_values <@ old_row then
      -- Idempotent retry of a create: the existing row already has exactly
      -- these values under this client-generated ID.
